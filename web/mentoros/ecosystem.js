...
(() => {
  ...
  async function request(path, { method = 'GET', body, raw = false, audioResponse = false, headers = {}, signal } = {}) {
    ...
    // Image councils include generation and peer review before synthesis.
    const timer = setTimeout(abort, path === '/api/ecosystem/council' ? 360000 : 240000);
    try {
  ...
  const imageProvenanceText = (provenance) => provenance?.model
    ? `${provenance.model} · ${provenance.quality || 'unspecified'} quality`
    : 'Older image · generation model not recorded';
  function renderControls() {
    if ($('#eco-image-config')) $('#eco-image-config').textContent = state?.imageGeneration
      ? `New images: ${state.imageGeneration.model} · ${state.imageGeneration.quality} quality`
      : 'Image model configuration is unavailable.';
  ...
  function councilImageReferences(value, run) {
    ...
    const text = String(value || '').replace(/!\[([^\]\r\n]*)\]\(\s*([^\s)]+)\s*\)/g, (markup, label, target) => {
      ...
      images.set(url, { url, label: label.trim() || 'Meal illustration generated by the council', mentorName: message.mentorName, provenance: message.imageProvenance });
      return ' ';
    }).replace(/[ \t]+\n/g, '\n').replace(/\n{3,}/g, '\n\n').trim();
  ...
  function renderDecision(run) {
    ...
    const figures = referencedImages.map((image) => `<figure class="eco-council-image eco-decision-image"><a href="${esc(image.url)}" target="_blank" rel="noopener noreferrer" aria-label="Open generated meal illustration"><img src="${esc(image.url)}" alt="${esc(image.label)}" loading="lazy"></a><figcaption>AI-generated illustration · ${esc(image.mentorName)}<span>${esc(imageProvenanceText(image.provenance))}</span><span>${esc(image.label)}</span></figcaption></figure>`).join('');
  ...
  async function generateMeal(event) {
    ...
    try {
      ...
      meal = result.image;
      region('#eco-meal-result', `<figure><img src="${esc(url)}" alt="${esc(meal.caption || `AI-generated meal: ${meal.prompt}`)}"><figcaption>AI-generated meal image<span>${esc(imageProvenanceText(meal.provenance))}</span><span>${esc(meal.caption || meal.prompt)}</span></figcaption></figure><button id="eco-search-ingredients" class="eco-button eco-secondary">Search ingredients with council</button><p class="eco-caption">Choose 2–4 mentors, including one with Web search enabled. Image generation does not establish nutrition or ingredient prices.</p>`);
