...
export interface ImageProvenance { provider: 'openai'; model: string; quality: 'high'; size: '1024x1024'; requestId?: string; }
export interface CouncilMessage { id: string; mentorId: string; mentorName: string; to: string; phase: 'proposal' | 'review' | 'resolution' | 'tool'; text: string; at: string; imageUrl?: string; imageProvenance?: ImageProvenance; }
...
export interface EcosystemState { mentors: CustomMentor[]; profile: SharedProfile; profileRevision: number; run: CouncilRun | null; memory: string[]; capabilities: {agents: boolean; search: boolean; vision: boolean; image: boolean}; imageGeneration?: Omit<ImageProvenance, 'provider' | 'requestId'>; }
...
export interface MealImage { url: string; prompt: string; caption: string; generatedAt: string; provenance?: ImageProvenance; }
